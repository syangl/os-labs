...
print_stackframe(void) {
      ...
      */
      ...
      for (int i = 0; i < STACKFRAME_DEPTH && ebp_value != 0; i++){
        cprintf("ebp:0x%08x eip:0x%08x ", ebp_value, eip_value);
        uint32_t* arg_ptr = (uint32_t*)(ebp_value + 8);
        cprintf("arguments:0x%08x 0x%08x 0x%08x 0x%08x\n", arg_ptr[0], arg_ptr[1], arg_ptr[2], arg_ptr[3]);
        print_debuginfo(eip_value - 1);
        eip_value = *((uint32_t*)(ebp_value + 4));
