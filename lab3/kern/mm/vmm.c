...
/*mm_struct变量，其中保存了所使用的PDT，合法的虚拟地址空间（使用链表组织），以及与后文的swap机制相关的数据
 *硬件产生的error code，可以用于帮助判断发生page fault的原因
 *出现page fault的线性地址
 */
int
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    int ret = -E_INVAL;
    // 查询mm_struct中的合法的虚拟地址，确定当前出现page fault的线性地址是否合法，如果合法则继续执行调出物理页，否则直接返回
    ...
    // 是否允许读写来判断是否出现了读/写不允许的这种情况，如果出现了上述情况，则应该直接返回
    switch (error_code & 3) {
    default:
...
#endif
/* *
在ucore中同时使用页表项来保存物理地址（在Present位为1的时候）以及被换出的物理页在swap外存中的位置
 * swap_entry_t
 * --------------------------------------------
 * |         offset        |   reserved   | 0 |
 * --------------------------------------------
 ...
 * */
    ...
    if (*ptep == 0){// 如果需要的物理页是没有分配而不是被换出到外存中
        ...
        { // 判断当前交换机制初始化正常
            ...
            page_insert(mm->pgdir, page, addr, perm); // 插入二级页表树结构，将物理页与虚拟页建立映射关系
