...
_fifo_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    //将当前的物理页插入FIFO可被交换出去的物理页链表末尾，链表中接近链表头的物理页面在内存中的驻留时间更长
...
_fifo_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     list_entry_t *le = list_next(head);             // 取出链表头第一个，即驻留时间最长的物理页
     assert(le != head);                             // 链表非空
     struct Page *page = le2page(le, pra_page_link); // 找到对应的物理页的page
     list_del(le);                                   // 从链表上删除取出的将被换出的物理页
     *ptr_page = page;
     return 0;
}
